...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if data_path.endswith(".csv"):
            ...
            df_raw = np.load(data_path, allow_pickle=True)

            # 如果需要 pandas DataFrame，可以转一下
            if isinstance(df_raw, np.ndarray):
                if df_raw.ndim == 1:
                    df_raw = pd.DataFrame(df_raw, columns=["value"])
                else:
            ...
            df_raw = pd.DataFrame(npz_file[first_key])
